...
class BaseAI(Action):
    ...
    def get_line_to(self, dest_x: int, dest_y: int) -> List[Tuple[int, int]]:
        """Compute and return a straight line from self to the target."""
        # TODO: i'm sure there's a better way to do this than just using an array filled with ones
        # Copy the tiles array and fill with ones.
        cost = np.array(self.entity.gamemap.tiles["walkable"], dtype=np.int8)

        # Create a graph from this array of ones and pass it to the pathfinder
        graph = tcod.path.SimpleGraph(cost=cost, cardinal=2, diagonal=3)
        pathfinder = tcod.path.Pathfinder(graph)
